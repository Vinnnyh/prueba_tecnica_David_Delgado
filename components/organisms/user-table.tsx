...
import { User, Shield, Mail, Calendar, Edit2, Check, X, Phone } from 'lucide-react';
...
interface UserTableProps {
  ...
  onCancelEdit: () => void;
  onSaveEdit: () => void;
  onEditValueChange: (values: { name: string; phone: string; roleId: string }) => void;
}
  ...
  return (
    <div className="bg-brand-card rounded-3xl border border-white/5 overflow-hidden flex flex-col min-h-[600px]">
        ...
        <table className="w-full border-collapse min-w-[1000px]">
          <thead>
            <tr className="border-b border-white/5 text-gray-500 text-[10px] uppercase tracking-[0.2em]">
              <th className="px-8 py-5 font-black text-left">User Information</th>
              <th className="px-8 py-5 font-black text-left">Contact Details</th>
                      ...
                      <div className="flex flex-col gap-1">
                        {isEditing ? (
                          <Input
                            ...
                            onChange={(e) => onEditValueChange({ ...editValues!, name: e.target.value })}
                            className="h-9 text-sm min-w-[180px]"
                            placeholder="Full Name"
                          />
                        ) : (
                          <p className="font-bold text-white text-base group-hover:text-brand-accent transition-colors">{u.name}</p>
                        )}
                        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">ID: {u.id.slice(0, 8)}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-8">
                      ...
                      <div className="flex items-center gap-2.5 text-sm text-gray-400 font-medium">
                        ...
                        {isEditing ? (
                          <Input
                            ...
                            onChange={(e) => onEditValueChange({ ...editValues!, phone: e.target.value })}
                            className="h-9 text-sm min-w-[180px]"
                            placeholder="Phone Number"
                          />
                        ) : (
                          <span className={u.phone ? 'text-gray-400' : 'text-gray-600 italic'}>
                    ...
                    <div className="flex justify-center">
                      {isEditing ? (
                        <Dropdown
                          ...
                          onChange={(val) => onEditValueChange({ ...editValues!, roleId: val })}
                  ...
                  <td className="px-8 py-8 text-right">
                    {isEditing ? (
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          {isSaving ? <LoadingDots className="w-8" /> : <Check size={18} />}
                        </Button>
                        <Button
      ...
      <div className="px-8 py-10 border-t border-white/5 bg-white/[0.01] rounded-b-3xl flex justify-center items-center">
        <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] opacity-20">User Management System</p>
      </div>
    </div>
  );
};