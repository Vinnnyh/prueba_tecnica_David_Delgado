                ...
                >
                  {isExporting ? (
                    <div className="absolute inset-0 flex items-center justify-center bg-brand-card/80 backdrop-blur-[2px]">
                      <LoadingDots className="w-10" dotClassName="bg-brand-accent" />
                    </div>
                  ) : (
                    <>
                      <Download size={16} />
        ...
        <table className="w-full border-collapse min-w-[900px]">
          <thead>
            <tr className="border-b border-white/5 text-gray-500 text-[10px] uppercase tracking-[0.2em]">
              ...
              {showUser && <th className="px-8 py-5 font-black text-center">User</th>}
          ...
          <tbody className="divide-y divide-white/5">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={showUser ? 5 : 4} className="px-8 py-24 text-center">
                  <div className="flex flex-col items-center gap-3 opacity-30">
                    <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center">
                      <span className="text-2xl">∅</span>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm font-bold text-white">No movements found</p>
              ...
              movements.map((m) => (
                <tr key={m.id} className="hover:bg-white/[0.02] transition-colors group">
                  <td className="px-8 py-5 font-bold text-white group-hover:text-brand-accent transition-colors">{m.concept}</td>
                  {showUser && (
                  ...
                  <td className="px-8 py-5 font-black text-right text-base">
                    <span className={m.type === 'INCOME' ? 'text-brand-income' : 'text-brand-expense'}>
        ...
        <div className="px-8 py-6 border-t border-white/5 bg-white/[0.01] rounded-b-3xl">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-8">
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6">
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Rows</span>
                <TableRowsSelector value={pageSize} onChange={onPageSizeChange} />
              </div>
              <div className="h-4 w-px bg-white/10 hidden sm:block" />
              <span className="text-xs text-gray-500 font-bold">
                Showing <span className="text-white">{startIndex + 1}</span> to <span className="text-white">{Math.min(startIndex + pageSize, totalCount)}</span> of <span className="text-white">{totalCount}</span>
              </span>
            </div>
